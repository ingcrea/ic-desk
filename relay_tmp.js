 ...
 *
 * Seguridad:
 *   - Agentes: cabecera  x-ic-agent-token: ICAgentToken2026SecureHashKey
...
const AGENT_TOKEN   = 'ICAgentToken2026SecureHashKey';
...
function handleAgent(ws, request, id) {
  ...
  const query = parseQuery(request.url);
  const token = request.headers['x-ic-agent-token'] || query.get('agent_token');
  ...
  ws.on('message', (raw, isBinary) => {
    ...
    // Si el mensaje es binario o comienza con 0xFF (JPEG), es un fotograma de video.
    // NUNCA debe procesarse como JSON, independientemente de si hay panel o no.
    if (isBinary || (Buffer.isBuffer(raw) && raw[0] !== 123)) {
      ...
      return; // Se descarta el frame silenciosamente si no hay panel (evita crashear la red)
    }
    ...
    if (session.panel && session.panel.readyState === WebSocket.OPEN) {
      session.panel.send(raw, { binary: isBinary }); // Mantener formato original (JSON si es Text)
    }
  });
