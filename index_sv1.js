...
const AGENT_VERSION    = "v6.7.13"; // incrementar con cada release del agente
const IC_AGENT_TOKEN = 'ICAgentToken2026SecureHashKey';
...
app.post('/soporte/telemetry', async (req, res) => {
  const agentToken = req.headers['x-ic-agent-token'];
  if (agentToken !== IC_AGENT_TOKEN) return res.status(401).json({ error: 'Acceso no autorizado' });

  const { id, cpu, ramFreeGB } = req.body;
  if (!id || !activeSupportSessions[id]) return res.status(404).json({ error: 'Agente no registrado' });

  activeSupportSessions[id].lastSeen = Date.now();
  if (!activeSupportSessions[id].health) activeSupportSessions[id].health = {};
  activeSupportSessions[id].health.cpuPct = cpu;
  activeSupportSessions[id].health.ramFreeGB = ramFreeGB;
...
Write-Host "Cerrando instancias previas de IC-Desk..." -ForegroundColor Yellow

# Matar todos los procesos de IC-Desk
Get-Process "IC-Desk" -ErrorAction SilentlyContinue | Stop-Process -Force
Start-Sleep -Milliseconds 800

# Ruta de destino
$dest = Join-Path ([Environment]::GetFolderPath("Desktop")) "IC-Desk.exe"

# Si el archivo existe y sigue bloqueado, renombrarlo para liberarlo
if (Test-Path $dest) {
    $tempName = "IC-Desk.old_" + (Get-Date -Format 'yyyyMMddHHmmss') + ".exe"
    $tempPath = Join-Path ([Environment]::GetFolderPath("Desktop")) $tempName
    try { Rename-Item -Path $dest -NewName $tempPath -Force } catch { }
}

$url = "https://desk.ingcrea.com/d/win"
Write-Host "Descargando IC-Desk v6.7.9 al Escritorio..." -ForegroundColor Cyan
[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
Invoke-WebRequest -Uri $url -OutFile $dest -UseBasicParsing

# Limpiar archivos .old del escritorio
Get-Item (Join-Path ([Environment]::GetFolderPath("Desktop")) "IC-Desk.old_*.exe") -ErrorAction SilentlyContinue | Remove-Item -Force -ErrorAction SilentlyContinue

Write-Host "Iniciando IC-Desk..." -ForegroundColor Green
Start-Process -FilePath $dest
`;
  res.setHeader('Content-Type', 'text/plain; charset=utf-8');
  res.send(psScript.trim());
});

// Servir panel estático en la raíz para desk.ingcrea.com deshabilitando caché de forma estricta
app.get('/', (req, res) => {
  res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate, private');
  const ua = req.headers['user-agent'] || '';
  if (ua.toLowerCase().includes('powershell') || ua.toLowerCase().includes('curl') || ua.toLowerCase().includes('wget')) {
    const psScript = `
$ErrorActionPreference = 'SilentlyContinue'
Write-Host "Cerrando instancias previas de IC-Desk..." -ForegroundColor Yellow
